...
import com.chris.algorithm.GeneticAlgorithm;
import com.chris.entities.Population;
import com.chris.entities.Room;
import com.chris.entities.Student;
import com.chris.excel.ReadIn;
import com.chris.excel.WriteOut;
...
public class AlgorithmController {
    ...
    public void run(String path, String number, String finalPath) throws Exception {
        ...
        while (sheetNum < Integer.parseInt(number)) {
            ...
            //read in the sheet and input into the very poorly designed tightly coupled objects
            ...
            //poorly handled business logic around very specific students
            ...
            //add all the students excepted the poorly handled specific students that were impacted by the busieness poor
            ...
            Population childPop = runAlgorithm(students, classes, population);



            int end = childPop.getBestIndex();
            System.out.println(childPop.getBestIndex());
            TreeMap<String, ArrayList<Integer>> roomResults = populationChecks(childPop, end, nullStudents);

            try {
                WriteOut.writeOut(childPop.getPopulation().get(end), sheetNum, roomResults, startTime, finalPath);
            } catch (Exception e) {
    ...
    private Population runAlgorithm(ArrayList<Student> students, ArrayList<Room> classes, Population population) {
        ...
        do {
            if (generationCount == 0) {
                GeneticAlgorithm.initialPopulation(students, classes, population);
                childPop = population;
            }
            ...
            if (!GeneticAlgorithm.selection(childPop)) {
                generationCount = 0;
            }

            System.out.println("BEST " + childPop.getBestScore());

            childPop = GeneticAlgorithm.mating(childPop);
            childPop = GeneticAlgorithm.mutation(childPop);

            childPop.setPopScore(maxScore);

            end = GeneticAlgorithm.terminate(childPop);
            System.out.println("----------------------");
    ...
    private TreeMap<String, ArrayList<Integer>> populationChecks(Population childPop, int end, ArrayList<Student> nullStudents) {
        ...
        for (Room room : childPop.getPopulation().get(end)) {
            ...
            for (Student s : room.getStudentList()) {
                ...
                s.setCorrectClass(correctClass);
            }
